...
export default function TripDetailsScreen() {
  ...
  const details = useMemo(() => {
    ...
    const start = saved.StartingLocation?.starting_location;
    const budget = saved.StartingLocation?.budget_input;
    const members = saved.StartingLocation?.members;
    const destination = saved.DestinationLocation;

    let dates = "";
    if (saved.StartingLocation?.travel_dates) {
      try {
        const t = JSON.parse(saved.StartingLocation.travel_dates);
  ...
  const highlights = saved.HighlightDetails || [];
  const itineraryEntries = Object.entries(saved.Itinerary || {});
  const weather = saved.Weather;
  const events = Object.entries(saved.Event || {});
  const hotels = saved.HotelsData || [];
          ...
          <View className="px-4 mb-3">
            ...
            {itineraryEntries.map(([day, items]: any) => (
              <View key={day} className="mb-5">
                <Text className="text-sm font-bold text-cyan-700 mb-2 uppercase tracking-widest">{day}</Text>
                {items.map((item: any, i: number) => {
                  ...
                  return (
                    <View key={i} className="bg-white rounded-2xl p-3 mb-3 border border-slate-100 shadow-sm flex-row">
                      {img ? (
                        <Image source={{ uri: img }} className="w-32 h-32 rounded-xl mr-3 bg-slate-100" />
                      ) : (
                      ...
                      <View className="flex-1 justify-center">
                        <Text className="text-sm font-bold text-amber-900 mb-1">{item.title}</Text>
                        <Text className="text-[11px] font-bold text-blue-800 mb-1">{item.activity}</Text>
                        <Text className="text-[10px] text-slate-500 leading-4" numberOfLines={3}>{item.about}</Text>
                      </View>
                    </View>
                  );
                })}
        ...
        {hotels.length > 0 && (
          <View className="mb-8">
            <View className="px-4 flex-row items-center mb-3">
              <Ionicons name="bed-outline" size={20} color="#0f766e" />
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} className="pl-4">
              {hotels.map((hotel: any, i: number) => (
                <View key={i} className="mr-4 w-52 bg-white rounded-2xl overflow-hidden border border-slate-100 shadow-sm">
                  <Image source={{ uri: hotel.image }} className="w-full h-32 bg-slate-100" />
                  <View className="p-3">
        ...
        {weather && (
          <View className="px-4 mb-8">
            <View className="bg-white rounded-3xl p-5 border border-teal-50 shadow-sm" style={{ backgroundColor: '#f0fdfa' }}>
              ...
              <Text className="text-xs font-bold text-teal-800 uppercase mb-2 tracking-wider">Travel Advices</Text>
              {Array.isArray(weather.advice) && weather.advice.map((tip: string, i: number) => (
                <View key={i} className="flex-row items-start mb-2">
                  <Text className="text-teal-600 mr-2">•</Text>
                  <Text className="flex-1 text-[11px] font-semibold text-slate-800 leading-4">{tip}</Text>
                </View>
              ))}
            </View>
          </View>
            ...
            {events.map(([title, data]: any, i: number) => {
              ...
              return (
                <View key={i} className="bg-white rounded-2xl p-3 mb-3 border border-slate-100 shadow-sm flex-row">
                  ...
                  <View className="flex-1 justify-center">
                    <Text className="text-sm font-bold text-pink-900 mb-1">{title}</Text>
                    <Text className="text-[11px] text-slate-600 leading-4">{data.description}</Text>
                  </View>
                </View>
              );
            })}
